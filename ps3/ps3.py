...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        #round the numbers:
        x_position = math.floor(pos.get_x())
        y_position = math.floor(pos.get_y())
        #position is always valid
        #dirt in tile cannot be negative
        if capacity <= self.tiles[(x_position, y_position)]:
             self.tiles[(x_position, y_position)] -= capacity
        else:
            self.tiles[(x_position, y_position)] = 0 
    ...
    def is_tile_cleaned(self, m, n):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        #no need to round, alredy integers        
    ...
    def is_position_in_room(self, pos):
        ...
        return (x_position, y_position) in self.tiles
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        """
        self.room = room
        #only allow speeds above 0
        if speed <= 0 :
            raise ValueError("Negative speed")
        else:
            self.speed = speed
        #only allow robots that clean
        if capacity < 0:
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        return (Position(random.uniform(0.0, self.width),
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        """
        return self.is_tile_furnished(math.floor(pos.get_x(), pos.get_y())) 
    ...
    def is_position_valid(self, pos):
        ...
        """
        return (self.is_position_in_room(self, pos)
                and not self.is_position_furnished(self, pos))
    ...
    def get_random_position(self):
        ...
        """
        while True:
            random_position = (Position(random.uniform(0.0, self.width),
                                        random.uniform(0.0, self.height)))
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        #new position = a position choosen randomly, respecting the speed 
        #if position is valid, update position and then clean the floor
        #if new position is not a valid position, choose another position
        new_position = (self.get_robot_position()
                        .get_new_position(self.get_robot_direction(),self.speed))
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, 
    ...
    robot_type: class of robot to be instantiated (e.g. StandardRobot or
                FaultyRobot)
    """
    #each trial contains a room and a group of robots with their specifications
    trials_time = []
    ...
    for trial in range(num_trials):
        ...
        robot_set = []
        for single_robot in range(num_robots):
        #since robot_type will always be StandardRobot
            robot_set.append(robot_type(room, speed, capacity))

        total_steps = 0
        #Loop until current_coverage is greater than min_coverage
        while True:
            for single_robot in range(len(robot_set)):
                robot_set[single_robot].update_position_and_clean()
            total_steps +=1
            current_coverage = room.get_num_cleaned_tiles()/room.get_num_tiles()
            if current_coverage >= min_coverage:
                break
        
        trials_time.append(total_steps)
